...
using roopkotha.vela.richml;

/**
 * \ingroup vela
 * \defgroup divml HTML like content support to display application controls.
 */
 ...
 */
public class roopkotha.vela.divml.DivDocument : roopkotha.doc.RoopDocument {
	int counter;
	InputStream? instrm;
	extring rawData;
	enum config {
	...
	void traverseContents(onubodh.XMLIterator*xit) {
		...
		if(xit.nextTag.equals_static_string("LI")) {
			ListItemContent vlc = new ListItemContent(&content, &href, false);
			contents.set(counter++, vlc);
		} else {
			DivContent vrc = new DivContent(&content, &href, false);
